...
def get_argparse():
    ...
    parser.add_argument(
        ...
        required=False,
        action="store_true",
        help="Includes ALL sequences. Adds filter status in second column of header ('-w') or in third column of length table ('-l')",
    )
    parser.add_argument(
        "-w",
        required=False,
    ...
    parser.add_argument(
        ...
        type=str,
        default="length.tsv",
        help="Write sequence length table",
    )
    parser.add_argument(
        "-j",
        required=False,
...
class SeqObj:
    ...
    def to_row(self, include_status=False):
        if include_status:
            return f">{self.header}\t{self.length}\t{self.get_status()}"
        return f"{self.header}\t{self.length}\n"
    ...
    def parse(self):
        if self.length_tsv:
            header = (
                ...
                else "element_id\tlength\tstatus"
            )
            length_fh = open(self.length_tsv, "w")
            length_fh.write(header)
        for header, sequence in yield_fasta(self.fasta_in_fn):
            ...
            elif self.length_tsv is True:
                length_fh.write(seq_obj.to_row(include_status=self.include_all))
            else:
                pass
