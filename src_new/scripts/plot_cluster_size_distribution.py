...
class DataObj:
    ...
    def add_cluster(self, proteome_count, cluster_size):
        ...
        self.counts_by_cluster_size_by_proteome_counts[proteome_count][cluster_size] = (
            self.counts_by_cluster_size_by_proteome_counts[proteome_count].get(
                cluster_size, 0
            )
            + 1
        )
        self.counts_by_cluster_size[cluster_size] = (
            self.counts_by_cluster_size.get(cluster_size, 0) + 1
        )
    ...
    def homogenise_counts(self):
        for proteome_count in self.counts_by_cluster_size_by_proteome_counts:
            for cluster_size in self.counts_by_cluster_size:
                if (
                    cluster_size
                    not in self.counts_by_cluster_size_by_proteome_counts[
                        proteome_count
                    ]
                ):
                    self.counts_by_cluster_size_by_proteome_counts[proteome_count][
        ...
        self.proteome_count_max = max(
            list(self.counts_by_cluster_size_by_proteome_counts.keys())
        )
    ...
    def yield_counts_by_proteome_count(self, count_type):
        ...
        for proteome_count in sorted(self.counts_by_cluster_size_by_proteome_counts):
            colour = cmap(1.0 * proteome_count / self.proteome_count_max)
        ...
        if plot_type in PLOTS:
            if (
                ...
                for (
                    ...
                    y_bottom,
                    colour,
                ) in self.yield_counts_by_proteome_count("absolute"):
                    x = np.array(x)
                    y = np.array(y)
                    ax.plot(x, y, c="None")
                    mask = np.where(y >= 1, y, np.nan)
                    ax.fill_between(
                        x, y, y_bottom, facecolor=colour, interpolate=True, where=mask
                    )
                ...
                for (
                    ...
                    y_bottom,
                    colour,
                ) in self.yield_counts_by_proteome_count("relative"):
                ...
                for (
                    ...
                    y_perc = [
                        (y_ / self.counts_by_cluster_size[cluster_size]) * 100
                        for y_, cluster_size in zip(
                            y, sorted(self.counts_by_cluster_size)
                        )
                    ]
                    y_bottom_perc = [
                        (y_b / self.counts_by_cluster_size[cluster_size]) * 100
                        for y_b, cluster_size in zip(
                            y_bottom, sorted(self.counts_by_cluster_size)
                        )
                    ]
            ...
            for colour, proteomes in self.proteome_counts_by_colour.items():
                if len(proteomes) > 1:
                    legend_handles.append(
                        mpatches.Patch(
                            label="%s-%s" % (proteomes[0], proteomes[-1]), color=colour
                        )
                    )
                else:
                    legend_handles.append(
                        mpatches.Patch(label="%s" % (proteomes[0]), color=colour)
                    )
            legend = ax.legend(
                ...
                numpoints=1,
                frameon=True,
                title="Number of proteomes in cluster",
            )
