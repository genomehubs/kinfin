...
def main(args):
    ...
    with open(fasta_f + ".filter_stats.txt", "w") as out_fh:
        out_fh.write(
            "filename=%s; raw=%s (%s); stop_fail=%s (%s); length_fail=%s (%s); pass=%s (%s)\n"
            % (
                fasta_f,
                stats["raw"],
                "{:.2%}".format(stats["raw"] / stats["raw"]),
                stats["stop_fail"],
                "{:.2%}".format(stats["stop_fail"] / stats["raw"]),
                stats["length_fail"],
                "{:.2%}".format(stats["length_fail"] / stats["raw"]),
                stats["pass"],
                "{:.2%}".format(stats["pass"] / stats["raw"]),
            )
        )
