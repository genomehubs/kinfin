...
from os import environ, getcwd, makedirs, mkdir, remove
from os.path import abspath, dirname, exists, isabs, isfile, join, realpath
...
try:
    ...
    import_errors.append(
        "[ERROR] : Module 'Docopt' was not found. Please install 'Docopt' using 'pip install docopt'"
    )
try:
    ...
    import_errors.append(
        "[ERROR] : Module 'Matplotlib' was not found. Please install 'Matplotlob' using 'pip install matplotlib'"
    )
try:
    import scipy
except ImportError:
    import_errors.append(
        "[ERROR] : Module 'SciPy' was not found. Please install 'SciPy' using 'pip install scipy'"
    )
try:
    import ete3
except ImportError:
    import_errors.append(
        "[ERROR] : Module 'ete3' was not found. Please install 'ete3' using 'pip install ete3'\nPlotting of trees requires additional dependencies:\n\t- PyQt4\n\t"
    )
if import_errors:
    sys.exit("\n".join(import_errors))
...
def get_ALO_cluster_cardinality(ALO_proteome_counts_in_cluster):
    if len(ALO_proteome_counts_in_cluster) > 2:
        ...
        if all(count == 1 for count in ALO_proteome_counts_in_cluster):
            ...
            ALO_proteome_counts_in_cluster_at_fuzzycount_count = len(
                [
                    ALO_proteome_counts
                    for ALO_proteome_counts in ALO_proteome_counts_in_cluster
                    if ALO_proteome_counts == inputObj.fuzzy_count
                ]
            )
            ALO_proteome_counts_in_cluster_in_fuzzyrange_count = len(
                [
                    ALO_proteome_counts
                    for ALO_proteome_counts in ALO_proteome_counts_in_cluster
                    if ALO_proteome_counts in inputObj.fuzzy_range
                ]
            )
            fuzzy_fraction = (
                ALO_proteome_counts_in_cluster_at_fuzzycount_count
                / ALO_proteome_counts_in_cluster_length
            )
            if fuzzy_fraction >= inputObj.fuzzy_fraction:
                if (
                    ALO_proteome_counts_in_cluster_at_fuzzycount_count
                    + ALO_proteome_counts_in_cluster_in_fuzzyrange_count
                    == ALO_proteome_counts_in_cluster_length
                ):
                    return "fuzzy"
    return None
...
def statistic(count_1, count_2, test):
    ...
    if (
        len(implicit_count_1) >= inputObj.min_proteomes
        and len(implicit_count_2) >= inputObj.min_proteomes
    ):
        ...
        if (
            ...
            pvalue = scipy.stats.ttest_ind(
                implicit_count_1, implicit_count_2, equal_var=False
            )[
                1
            ]  # Welch's t-test
            ...
            try:
                pvalue = scipy.stats.mannwhitneyu(
                    implicit_count_1, implicit_count_2, alternative="two-sided"
                )[1]
            ...
            pvalue = scipy.stats.ks_2samp(implicit_count_1, implicit_count_2)[
                1
            ]  # H0 that they are drawn from the same distribution
            if pvalue != pvalue:  # testing for "nan"
                pvalue = 1.0
        elif test == "kruskal":
            pvalue = scipy.stats.kruskal(implicit_count_1, implicit_count_2)[
                1
        ...
        for domain_source, mapping_f in list(mapping_file_by_domain_source.items()):
            if domain_source == "Pfam":
                ...
                for line in read_file(mapping_f):
                    ...
                    if (
                        domain_id
                        not in domain_description_by_domain_id_by_domain_source[
                            domain_source
                        ]
                    ):
                        domain_description_by_domain_id_by_domain_source[domain_source][
                        ...
                        if (
                            ...
                            sys.exit(
                                "[ERROR] : Conflicting descriptions for %s"
                                % (domain_id)
                            )
                ...
                for line in read_file(mapping_f):
                    if not line.startswith("!"):
                        ...
                        go_desc, go_id = go_string[0].replace("GO:", ""), go_string[
                            1
                        ].lstrip(" ")
                        if (
                            go_id
...
def parse_tree(tree_f, outgroups):
    ...
    if len(outgroups) > 1:
        ...
        try:
            ...
            print(
                "[STATUS] - Setting LCA of %s as outgroup : ..." % (",".join(outgroups))
            )
...
class DataFactory:
    ...
    def build_AloCollection(self):
        ...
        ) = self.parse_attributes(config_f)
        # Add taxonomy if needed
        if "TAXID" in set(attributes):
            print(
                "[STATUS] - Attribute 'TAXID' found, inferring taxonomic ranks from nodesDB..."
            )
            attributes, level_by_attribute_by_proteome_id = self.add_taxid_attributes(
                nodesdb_f, attributes, level_by_attribute_by_proteome_id
            )
        ...
        if tree_f:
            ...
            if "OUT" not in attributes:
                sys.exit(
                    "[ERROR] - Please specify one of more outgroup taxa in the config file."
                )
            outgroups = [
                proteome_id
        ...
        for line in read_file(config_f):
            if line.startswith("#"):
                if not attributes:
                    ...
                    if (
                        not attributes[0].upper() == "IDX"
                        or not attributes[1].upper() == "TAXON"
                    ):
                        sys.exit(
                            "[ERROR] - First/second element have to be IDX/TAXON.\n\t%s"
                            % (attributes)
                        )
                else:
                ...
                if not len(temp) == len(attributes):
                    sys.exit(
                        "[ERROR] - number of columns in line differs from header\n\t%s\n\t%s"
                        % (attributes, temp)
                    )
                if temp[1] in proteomes:
                    sys.exit(
                        "[ERROR] - 'TAXON' should be unique. %s was encountered multiple times"
                        % (temp[0])
                    )
                species_id = temp[0]
                proteome_id = temp[1]
    ...
    def add_taxid_attributes(
        self, nodesdb_f, attributes, level_by_attribute_by_proteome_id
    ):
    ...
    def build_ProteinCollection(self, inputObj):
        ...
        for line in read_file(sequence_ids_f):
            ...
            if proteome_id:
                proteinObj = ProteinObj(
                    protein_id, proteome_id, species_id, sequence_id
                )
        ...
        if fasta_dir:
            ...
            fasta_len_by_protein_id = self.parse_fasta_dir(
                fasta_dir, fasta_file_by_species_id
            )
        ...
        if functional_annotation_f:
            # PARSE DOMAINS
            print(
                "[STATUS] - Parsing %s ... this may take a while"
                % (functional_annotation_f)
            )
            for line in read_file(functional_annotation_f):
                ...
                if temp[0].startswith("#"):
                    ...
                    if not proteinCollection.domain_sources:
                        sys.exit(
                            "[ERROR] - %s does not seem to have a header."
                            % (functional_annotation_f)
                        )
                        ...
                        if not field == "None":
                            ...
                            for domain_id_count in domain_string:
                                ...
                                if domain_source == "GO":
                                    ...
                                    domain_id, domain_count = domain_id_count.rsplit(
                                        ":", 2
                                    )
                                domain_counts_by_domain_id[domain_id] = int(
                                    domain_count
                                )
                            domain_counter = Counter(domain_counts_by_domain_id)
                            domain_counter_by_domain_source[domain_source] = (
                                domain_counter
                            )
                    proteinCollection.add_annotation_to_proteinObj(
                        domain_protein_id, domain_counter_by_domain_source, go_terms
                    )
            ...
            proteinCollection.domain_description_by_domain_id_by_domain_source = (
                parse_mapping(mapping_file_by_domain_source)
            )
    ...
    def build_ClusterCollection(self, inputObj):
        ...
        with open(cluster_f) as fh:
            for line in fh:
                ...
                protein_string = [
                    protein_id for protein_id in protein_string if protein_id
                ]
    ...
    def plot_cluster_sizes(self):
        ...
        count_plot_f = join(
            self.dirs["main"], "cluster_size_distribution.%s" % (inputObj.plot_format)
        )
    ...
    def get_header_line(self, filetype, attribute):
        if filetype == "attribute_metrics":
            ...
            cluster_metrics_header += [
                "%s_count" % level
                for level in sorted(aloCollection.ALO_by_level_by_attribute[attribute])
            ]
            if not attribute.upper() == "TAXON":
                cluster_metrics_header += [
                    "%s_median" % level
                    for level in sorted(
                        aloCollection.ALO_by_level_by_attribute[attribute]
                    )
                ]
                cluster_metrics_header += [
                    "%s_cov" % level
            ...
            for level in sorted(
                aloCollection.ALO_by_level_by_attribute.get(
                    "TAXON", aloCollection.ALO_by_level_by_attribute.get("taxon", {})
                )
            ):
    ...
    def write_cluster_metrics(self):
        ...
        cluster_metrics_domains_f = join(
        ...
        for domain_source in clusterCollection.domain_sources:
            ...
            cluster_metrics_domains_detailed_f_by_domain_source[domain_source] = join(
                self.dirs["main"], "cluster_domain_annotation.%s.txt" % (domain_source)
            )

        for attribute in aloCollection.attributes:

            attribute_metrics_f = join(
                self.dirs[attribute], "%s.attribute_metrics.txt" % (attribute)
            )
            ...
            pairwise_representation_test_f = join(
                self.dirs[attribute],
                "%s.pairwise_representation_test.txt" % (attribute),
            )
            ...
            cluster_metrics_f = join(
                self.dirs[attribute], "%s.cluster_summary.txt" % (attribute)
            )
            ...
            levels = sorted(
                [x for x in aloCollection.ALO_by_level_by_attribute[attribute]]
            )
            levels_seen = set()
            ...
            for level in levels:
                ...
                cluster_metrics_ALO_f = join(
                    self.dirs[attribute],
                    "%s.%s.cluster_metrics.txt" % (attribute, level),
                )
                ...
                cluster_1to1_ALO_f = join(
                    self.dirs[attribute], "%s.%s.cluster_1to1s.txt" % (attribute, level)
                )
                        ...
                        for (
                            ...
                            for (
                                ...
                                cluster_1to1_ALO_line.append(
                                    clusterCollection.clusterObjs_by_cluster_id[
                                        cluster_id
                                    ].proteome_count
                                )
                                cluster_1to1_ALO_line.append(
                                    "{0:.2f}".format(
                                        len(
                                            [
                                                ...
                                                if protein_count == inputObj.fuzzy_count
                                            ]
                                        )
                                        / clusterCollection.clusterObjs_by_cluster_id[
                                            cluster_id
                                ...
                                cluster_1to1_ALO_output.append(
                                    "\t".join(
                                        [str(field) for field in cluster_1to1_ALO_line]
                                    )
                                )
                ...
                for clusterObj in clusterCollection.clusterObjs:
                    ...
                    if not levels_seen:
                        ...
                        cluster_metrics_line.append(
                            clusterObj.cluster_type_by_attribute[attribute]
                        )
                        if clusterCollection.fastas_parsed:
                            cluster_metrics_line.append(
                                clusterObj.protein_length_stats["mean"]
                            )
                            cluster_metrics_line.append(
                                clusterObj.protein_length_stats["sd"]
                            )
                        else:
                        ...
                        for _level in levels:
                            cluster_metrics_line.append(
                                sum(
                                    clusterObj.protein_counts_of_proteomes_by_level_by_attribute[
                                        attribute
                                    ][
                                        _level
                                    ]
                        ...
                        if not attribute.upper() == "TAXON":
                            ...
                            for _level in levels:
                                cluster_metrics_line.append(
                                    "{0:.2f}".format(
                                        clusterObj.proteome_coverage_by_level_by_attribute[
                                            attribute
                                        ][
                                            _level
                                        ]
                    ...
                    if not levels_seen and attribute.upper() == "TAXON":
                        cafe_line = []
                    ...
                    if not levels_seen and attribute.upper() == "TAXON":
                        if clusterCollection.functional_annotation_parsed:
                            ...
                            cluster_metrics_domains_line.append(
                                clusterObj.protein_count
                            )
                            cluster_metrics_domains_line.append(
                                clusterObj.proteome_count
                            )
                            if clusterCollection.fastas_parsed:
                                cluster_metrics_domains_line.append(
                                    clusterObj.protein_length_stats["mean"]
                            ...
                            if "SignalP_EUK" in clusterCollection.domain_sources:
                                cluster_metrics_domains_line.append(
                                    "{0:.2f}".format(
                                        clusterObj.secreted_cluster_coverage
                                    )
                                )
                            else:
                                cluster_metrics_domains_line.append("N/A")
                            for domain_source in clusterCollection.domain_sources:
                                ...
                                if (
                                    ...
                                    cluster_metrics_domains_line.append(
                                        "{0:.3f}".format(
                                            clusterObj.domain_entropy_by_domain_source[
                                                domain_source
                                            ]
                                        )
                                    )
                            ...
                            for (
                                ...
                                for (
                                    ...
                                    if domain_source == "SignalP_EUK":
                                        ...
                                        if (
                                            domain_source
                                            in proteinCollection.domain_description_by_domain_id_by_domain_source
                                        ):
                                            cluster_metrics_domains_detailed_output_line.append(
                                                proteinCollection.domain_description_by_domain_id_by_domain_source[
                                                    domain_source
                                    ...
                                    for proteome_id, protein_ids in list(
                                        clusterObj.protein_ids_by_proteome_id.items()
                                    ):
                                        proteome_seen = False
                                        for protein_id in protein_ids:
                                            if (
                                                domain_source
                                                in proteinCollection.proteinObjs_by_protein_id[
                                                    protein_id
                                                ].domain_counter_by_domain_source
                                                and domain_id
                                                in proteinCollection.proteinObjs_by_protein_id[
                                        ...
                                        sorted(
                                            [
                                                ...
                                                % (
                                                    ...
                                                    count,
                                                    len(
                                                        clusterObj.protein_ids_by_proteome_id[
                                                            proteome_id
                                                        ]
                                                    ),
                                                )
                    ...
                    cluster_metrics_ALO_line.append(
                        ALO.cluster_status_by_cluster_id[clusterObj.cluster_id]
                    )
                    cluster_metrics_ALO_line.append(
                        ALO.cluster_type_by_cluster_id[clusterObj.cluster_id]
                    )
                    ...
                    if ALO.cluster_mean_non_ALO_count_by_cluster_id[
                        clusterObj.cluster_id
                    ]:
                        cluster_metrics_ALO_line.append(
                    ...
                    ALO_proteomes_present = clusterObj.proteome_ids.intersection(
                        ALO.proteomes
                    )
                    non_ALO_proteomes_present = clusterObj.proteome_ids.difference(
                        ALO.proteomes
                    )
                    ...
                    if (
                        len(levels) > 1
                        and len(ALO_proteomes_present) >= inputObj.min_proteomes
                    ):
                        for result in self.pairwise_representation_test(
                            clusterObj, attribute, level, levels_seen, levels
                        ):
                ...
                if background_representation_test_by_pair_by_attribute:
                    self.plot_count_comparisons_vulcano(
                        background_representation_test_by_pair_by_attribute
                    )
    ...
    def plot_count_comparisons_vulcano(
        self, pairwise_representation_test_by_pair_by_attribute
    ):
        # [clusterObj.cluster_id, level, other_level, mean_level, mean_other_level, log2fc_mean, pvalue]
        for attribute in pairwise_representation_test_by_pair_by_attribute:
            for pair in pairwise_representation_test_by_pair_by_attribute[attribute]:
                ...
                if p_values:
                    pairwise_representation_test_f = join(
                        self.dirs[attribute],
                        "%s.pairwise_representation_test.%s.%s"
                        % (attribute, "_".join(pair_list), inputObj.plot_format),
                    )
                    ...
                    ooOne_artist = plt.Line2D(
                        (0, 1), (0, 0), color="red", linestyle="--"
                    )
                    # bonferroni
                    ...
                    v1_label = "|log2FC| = 1" % ()
                    v1_artist = plt.Line2D(
                        (0, 1), (0, 0), color="purple", linestyle="--"
                    )
                    ...
                    legend = axScatter.legend(
                        ...
                        fontsize=inputObj.plot_font_size,
                        frameon=True,
                    )
                    ...
                    axScatter.set_xlabel(
                        "log2(mean(%s)/mean(%s))" % (x_label, y_label),
                        fontsize=inputObj.plot_font_size,
                    )
                    ...
                    plt.savefig(
                        pairwise_representation_test_f, format=inputObj.plot_format
                    )
                    plt.close()
    ...
    def pairwise_representation_test(
        ...
        for other_level in set(levels).difference(levels_seen):
            if not other_level == level:
                other_ALO = aloCollection.ALO_by_level_by_attribute[attribute][
                    other_level
                ]
...
class AloCollection:
    def __init__(
        ...
        self.attributes = [
            ...
            if attribute not in inputObj.ATTRIBUTE_RESERVED
        ]  # list of attributes
    ...
    def compute_proteomes_by_level_by_attribute(self):
        proteomes_by_level_by_attribute = {
    ...
    def analyse_clusters(self):
        if clusterCollection.inferred_singletons_count:
            print(
                "[STATUS]\t - Clusters found = %s (of which %s were inferred singletons)"
                % (
                    clusterCollection.cluster_count,
                    clusterCollection.inferred_singletons_count,
                )
            )
        else:
            print(
                "[STATUS]\t - Clusters found = %s" % (clusterCollection.cluster_count)
            )
    ...
    def analyse_cluster(self, clusterObj):
                    ...
                    if clusterObj.singleton is True:
                        ...
                        if clusterObj.proteome_count == 1:
                            ...
                            for child_node in node.get_children():
                                if child_node.proteome_ids.isdisjoint(
                                    ...
                                    child_node_proteome_ids_covered_count = len(
                                        clusterObj.proteome_ids.intersection(
                                    ...
                                    child_node_proteome_coverage_strings.append(
                                        ...
                                        % (
                                            child_node.name,
                                            child_node_proteome_ids_covered_count,
                                            len(child_node.proteome_ids),
                                        )
                                    )
                            if all(child_nodes_covered):
                                ...
                                node.synapomorphic_cluster_counts[
                                    node_cluster_type
                                ] += 1
        ...
        for attribute in self.attributes:
            ...
            for level in self.ALO_by_level_by_attribute[attribute]:
                ...
                for proteome_id in self.ALO_by_level_by_attribute[attribute][
                    level
                ].proteomes_list:
                ...
                protein_length_stats_by_level[level] = (
                    proteinCollection.get_protein_length_stats(
                        protein_ids_by_level[level]
                    )
                )
            ...
            for level in self.ALO_by_level_by_attribute[attribute]:
                ...
                if (
                    level
                    not in implicit_protein_ids_by_proteome_id_by_level_by_attribute[
                        attribute
                    ]
                ):
                    ...
                    if not cluster_type_by_attribute[attribute] == "singleton":
                        ALO_proteome_counts_in_cluster = [
                            count
                            for proteome_id, count in list(
                                explicit_protein_count_by_proteome_id_by_level[
                                    level
        ...
        clusterObj.protein_median = median(
            [
                count
        ...
        clusterObj.implicit_protein_ids_by_proteome_id_by_level_by_attribute = (
            implicit_protein_ids_by_proteome_id_by_level_by_attribute
        )
    ...
    def write_tree(self):
        if self.tree_ete:
            ...
            for node in self.tree_ete.traverse("levelorder"):
                ...
                node_stats.append(
                    "\t".join([str(string) for string in node_stats_line])
                )
                if inputObj.render_tree:
    ...
    def plot_tree(self, header_f_by_node_name, charts_f_by_node_name):
        tree_f = join(
            dataFactory.dirs["tree"], "tree.%s" % ("pdf")
    ...
    def generate_header_for_node(self, node):
        node_header_f = join(
            dataFactory.dirs["tree_headers"], "%s.header.png" % (node.name)
        )
        data = []
        data.append(
            (
                "Apomorphies (size=1)",
                "{:,}".format(node.apomorphic_cluster_counts["singletons"]),
            )
        )
        data.append(
            (
                "Apomorphies (size>1)",
                "{:,}".format(node.apomorphic_cluster_counts["non_singletons"]),
            )
        )
        data.append(
            (
        ...
        data.append(
            (
                ...
                "{:,}".format(node.synapomorphic_cluster_counts["complete_presence"]),
            )
        )
        data.append(
            (
                "Synapomorphies (cov<100%)",
                "{:,}".format(node.synapomorphic_cluster_counts["partial_absence"]),
            )
        )
    ...
    def generate_chart_for_node(self, node):
        ...
        if proteome_coverages:
            chart_f = join(
                dataFactory.dirs["tree_charts"], "%s.barchart.png" % (node.name)
            )
            ...
            if inputObj.plot_format == "pdf":
                pdf_chart_f = join(
                    dataFactory.dirs["tree_charts"], "%s.barchart.pdf" % (node.name)
                )
    ...
    def compute_rarefaction_data(self):
                ...
                if not len(proteome_ids) == 1:
                    ...
                    for repetition in range(0, inputObj.repetitions):
                        ...
                        for idx, proteome_id in enumerate(random_list_of_proteome_ids):
                            proteome_ALO = self.ALO_by_level_by_attribute.get(
                                "TAXON", self.ALO_by_level_by_attribute.get("taxon", {})
                            ).get(proteome_id, None)
                            if proteome_ALO:
        ...
        for attribute in rarefaction_by_samplesize_by_level_by_attribute:
            rarefaction_plot_f = join(
                dataFactory.dirs[attribute],
                "%s.rarefaction_curve.%s" % (attribute, inputObj.plot_format),
            )
            ...
            ax.set_ylabel(
                "Count of non-singleton clusters", fontsize=inputObj.plot_font_size
            )
...
class AttributeLevelObj:
    ...
    def __init__(self, attribute, level, proteomes):
        ...
        self.proteomes = set(
            proteomes
        )  # frozenset(), used for checking whether cluster and ALO intersect
        ...
        self.protein_ids_by_cluster_type = {
            "singleton": [],
            "specific": [],
            "shared": [],
        }  # list of lists
        self.protein_span_by_cluster_type = {
            "singleton": [],
            "specific": [],
            "shared": [],
        }
    ...
    def analyse_domains(self):
        ...
        domain_counter_by_domain_source_by_cluster_type = {
            "singleton": {},
            "specific": {},
            "shared": {},
        }
        ...
        for cluster_type in self.protein_ids_by_cluster_type:
            for domain_source in proteinCollection.domain_sources:
                if (
                    domain_source
                    not in domain_counter_by_domain_source_by_cluster_type[cluster_type]
                ):
                    domain_counter_by_domain_source_by_cluster_type[cluster_type][
                        domain_source
                    ] = Counter()
                    protein_with_domain_count_by_domain_source_by_cluster_type[
                        cluster_type
                ...
                for protein_id in self.protein_ids_by_cluster_type[cluster_type]:
                    ...
                    if domain_source in proteinObj.domain_counter_by_domain_source:
                        domain_counter = proteinObj.domain_counter_by_domain_source[
                            domain_source
                        ]
                        if domain_counter:
                            domain_counter_by_domain_source_by_cluster_type[
                                cluster_type
                            ][domain_source] += domain_counter
                            protein_with_domain_count_by_domain_source_by_cluster_type[
                                cluster_type
                            ][domain_source] += 1
        ...
        self.domain_counter_by_domain_source_by_cluster_type = (
            domain_counter_by_domain_source_by_cluster_type
        )
        self.protein_with_domain_count_by_domain_source_by_cluster_type = (
            protein_with_domain_count_by_domain_source_by_cluster_type
        )
        # print self.level
    ...
    def add_clusterObj(
        ...
        self.protein_length_stats_by_cluster_id[clusterObj.cluster_id] = (
            ALO_protein_length_stats
        )

        self.protein_count_by_cluster_id[clusterObj.cluster_id] = len(
            ALO_protein_ids_in_cluster
        )
        if ALO_cluster_status == "present":
            ...
            if not attribute_cluster_type == "singleton":
                if ALO_cluster_cardinality:
                    self.clusters_by_cluster_cardinality_by_cluster_type[
                        attribute_cluster_type
                    ][ALO_cluster_cardinality].append(clusterObj.cluster_id)
        ...
        self.cluster_mean_ALO_count_by_cluster_id[clusterObj.cluster_id] = (
            mean_ALO_count
        )
        self.cluster_mean_non_ALO_count_by_cluster_id[clusterObj.cluster_id] = (
            mean_non_ALO_count
        )
    ...
    def get_protein_count_by_cluster_type(self, cluster_type):
        if cluster_type == "total":
            return sum(
                [
                    len(protein_ids)
                    for cluster_type, protein_ids in list(
    ...
    def get_cluster_count_by_cluster_status_by_cluster_type(
        self, cluster_status, cluster_type
    ):
        if cluster_type == "total":
            return sum(
                [
                    len(cluster_ids)
                    for cluster_type, cluster_ids in list(
                        self.cluster_ids_by_cluster_type_by_cluster_status[
                            cluster_status
                        ].items()
                    )
    ...
    def get_cluster_count_by_cluster_cardinality_by_cluster_type(
        self, cluster_type, cluster_cardinality
    ):
        return len(
...
class ProteinCollection:
    def __init__(self, proteinObjs):
        ...
        self.proteinObjs_by_protein_id = {
            proteinObj.protein_id: proteinObj for proteinObj in proteinObjs
        }
    ...
    def add_annotation_to_proteinObj(
        self, domain_protein_id, domain_counter_by_domain_source, go_terms
    ):
        ...
        if proteinObj:
            ...
            signalp_notm = proteinObj.domain_counter_by_domain_source.get(
                "SignalP_EUK", None
            )
    ...
    def get_protein_length_stats(self, protein_ids):
        ...
        if protein_ids and self.fastas_parsed:
            protein_lengths = [
                self.proteinObjs_by_protein_id[protein_id].length
                for protein_id in protein_ids
            ]
...
class ProteinObj:
    ...
    def compute_domain_count_by_domain_id_by_domain_source(self):
        if self.domain_list:
            domain_ids_by_domain_source = {
                domainObj.domain_source: [] for domainObj in self.domain_list
            }
            for domainObj in self.domain_list:
                domain_ids_by_domain_source[domainObj.domain_source].append(
                    domainObj.domain_id
                )
            self.domain_count_by_domain_id_by_domain_source = {
                domain_source: Counter(domain_ids_by_domain_source[domain_source])
                for domain_source in domain_ids_by_domain_source
            }
        else:
...
class ClusterCollection:
    def __init__(
        ...
        self.clusterObjs_by_cluster_id = {
            clusterObj.cluster_id: clusterObj for clusterObj in clusterObjs
        }  # only for testing
...
class ClusterObj:
    def __init__(self, cluster_id, protein_ids):
        ...
        try:
            self.proteomes_by_protein_id = {
                protein_id: proteinCollection.proteinObjs_by_protein_id[
                    protein_id
                ].proteome_id
                for protein_id in protein_ids
            }
            ...
            sys.exit(
                "[ERROR] - Protein %s in clustering belongs to proteomes that are not present in the config-file. Please add those proteomes or recluster by omitting these proteomes."
                % (e.args[0])
            )
        ...
        self.domain_entropy_by_domain_source = (
    ...
    def compute_protein_length_stats(self):
        protein_lengths = [
            proteinCollection.proteinObjs_by_protein_id[protein_id].length
            for protein_id in self.protein_ids
        ]
    ...
    def compute_domain_counter_by_domain_source(self):
        ...
        for protein_id in self.protein_ids:
            protein_domain_counter_by_domain_source = (
                proteinCollection.proteinObjs_by_protein_id[
                    protein_id
                ].domain_counter_by_domain_source
            )
    ...
    def compute_go_terms(self):
        ...
        for protein_id in self.protein_ids:
            if proteinCollection.proteinObjs_by_protein_id[protein_id].go_terms:
                for go_term in proteinCollection.proteinObjs_by_protein_id[
                    protein_id
                ].go_terms:
...
class InputObj:
    def __init__(self, args):
        ...
        self.fuzzy_range = set(
            [
                x
                for x in range(self.fuzzy_min, self.fuzzy_max + 1)
                if not x == self.fuzzy_count
            ]
        )
        # values: rarefaction
        ...
        # taxrank
        self.taxranks = [
            taxrank.replace(" ", "") for taxrank in args["--taxranks"].split(",")
        ]
        self.check_taxranks()
    ...
    def check_test(self):
        ...
        if self.test not in SUPPORTED_TESTS:
            sys.exit(
                "[ERROR] : test %s is not supported (supported tests: %s)"
                % (self.test, ",".join(SUPPORTED_TESTS))
            )
    ...
    def check_plot_format(self):
        ...
        if self.plot_format not in SUPPORTED_PLOT_FORMATS:
            sys.exit(
                "[ERROR] : Plot format %s not part of supported plot formats (%s)"
                % (self.plot_format, ",".join(SUPPORTED_PLOT_FORMATS))
            )
    ...
    def check_taxranks(self):
        ...
        if unsupported_taxranks:
            sys.exit(
                "[ERROR] : Taxrank(s) %s not part of supported Taxranks (%s)"
                % (
                    ",".join(sorted(unsupported_taxranks)),
                    ",".join(sorted(SUPPORTED_TAXRANKS)),
                )
            )
    ...
    def check_if_fasta_dir_and_species_ids_f(self):
        if self.fasta_dir:
            if not self.species_ids_f:
                sys.exit(
                    "[ERROR] : You have provided a FASTA-dir using '--fasta-dir'. Please provide a Species-ID file using ('--species_ids_file')."
                )
    ...
    def check_input_files(self):
        ...
        if not isfile(nodesdb_f):
            if not isfile(nodesdb_gz):
                sys.exit(
                    "[ERROR] : nodesdb.gz could not be found in kinfin/data/ folder. Please download KinFin again.."
                )
            else:
        ...
        if self.pfam_mapping:
            pfam_mapping_f = join(
                dirname(realpath(__file__)), "../data/Pfam-A.clans.tsv.gz"
            )
            if not isfile(pfam_mapping_f):
                print(
                    "[ERROR] - PFAM-ID file 'data/Pfam-A.clans.tsv.gz' not found. Please run the install script to download"
                )
                sys.exit()
        ...
        if self.ipr_mapping:
            ...
            if not isfile(ipr_mapping_f):
                print(
                    "[ERROR] - IPR-ID file 'data/entry.list' not found. Please run the install script to download"
                )
                sys.exit()
            ...
            if not isfile(go_mapping_f):
                print(
                    "[ERROR] - GO-ID file, but 'data/interpro2go' not found. Please run the install script to download"
                )
                sys.exit()
    ...
    def check_that_ete_can_plot(self):
        if self.render_tree:
            try:
                ...
                sys.exit(
                    "[ERROR] : Plotting of trees requires additional ETE3 dependencies. PyQt4 is not installed. Please install PyQt4"
                )
            if "DISPLAY" in environ:
                ...
                try:
                    ...
                    print(
                        "[STATUS] - ETE can connect to X server (X11). Tree will be rendered."
                    )
                except Exception:
                    self.render_tree = False
                    print(
                        "[WARN] - ETE cannot connect to X server (X11). No tree will be rendered."
                    )
                remove(test_tree_f)
            else:
                print(
                    "[STATUS] - No X server found. ETE can't render the tree. Consider using 'xvfb-run' ..."
                )
                self.render_tree = False
    ...
    def check_fuzzy_count(self, target_count):
        if int(target_count) > 0:
            ...
            sys.exit(
                "[ERROR] : --target_count %s must be greater than 0" % (target_count)
            )
    ...
    def check_fuzzy_fraction(self, fuzzyness):
        if 0 <= float(fuzzyness) <= 1:
            ...
            sys.exit(
                "[ERROR] : --target_fraction %s is not between 0.0 and 1.0"
                % (fuzzyness)
            )
    ...
    def check_fuzzy_min_max(self, fuzzy_min, fuzzy_max):
        if int(fuzzy_min) <= int(fuzzy_max):
            ...
            sys.exit(
                "[ERROR] : --min %s is greater than --max %s" % (fuzzy_min, fuzzy_max)
            )
